...
public final class GoBackend implements Backend {
    ...
    /**
     * Apply a WireGuard userspace configuration to the running device without
     * recreating Android's TUN interface. FlowLink uses this for endpoint and
     * UDP-port changes on the same server profile.
     */
    public synchronized State updateConfig(final Tunnel tunnel, final Config config)
            throws Exception {
        if (currentTunnel != tunnel || currentTunnelHandle < 0)
            return setState(tunnel, State.UP, config);
        final int result = wgSetConfig(currentTunnelHandle,
                config.toWgUserspaceString());
        if (result != 0)
            throw new IllegalStateException("WireGuard hot update failed: " + result);
